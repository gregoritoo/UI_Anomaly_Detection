...
class Thread_train_model(threading.Thread):
    ...
    def __init__(self,look_back,len_prediction,model,q,x_train,y_train,nb_epochs,nb_batch,name_ts,name=''):
    ...
    def run(self):
        ...
        prediction=self.make_prediction(model,self.x_train,self.look_back)
        self.q.put(model)
    ...
    def make_prediction(self,model,x_train,look_back) :
        data=x_train[-1,:]
        prediction=np.zeros((1,self.len_prediction))
        for i in range (self.len_prediction) :
            dataset = np.reshape(data,(1, 1,look_back))
            predic=model.predict(dataset)
            data_residual=np.append(data[1 :],[predic]).reshape(-1,1)
            prediction[0,i]=predic
        return prediction
...
class New_Predictor(Predictor):
    ...
    def __init__(self,df,host,measurement,look_back,nb_layers,loss,metric,nb_features,optimizer,nb_epochs,nb_batch,form,freq_period) :
        ...
        self.form=form
        self.len_prediction=look_back*12
        self.look_back=look_back
        ...
        que = queue.Queue()
        threads_list = list()
        thread = Thread_train_model(self.look_back,self.len_prediction,model_trend,que,trend_x,trend_y,nb_epochs,nb_batch,"trend","Trend Thread")
        thread.start()
        threads_list.append(thread)
        thread_1 = Thread_train_model(self.look_back,self.len_prediction,model_seasonal,que,seasonal_x,seasonal_y,nb_epochs,nb_batch,"seasonal","Seasonal Thread")
        thread_1.start()
        threads_list.append(thread_1)
        thread_2= Thread_train_model(self.look_back,self.len_prediction,model_residual,que,residual_x,residual_y,nb_epochs,nb_batch,"residual","Residual Thread")
        thread_2.start()
        ...
        prediction_trend=que.get(block=False)
        self.model_seasonal=que.get(block=False)
        self.model_save(self.model_seasonal,"seasonal")
        prediction_seasonal=que.get(block=False)
        self.model_residual=que.get(block=False)
        self.model_save(self.model_residual,"residual")
        prediction_residual=que.get(block=False)
        self.prediction=prediction_trend+prediction_seasonal+prediction_residual
